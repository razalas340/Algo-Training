...
const needleInHaystack = (string, substring) => {
  for (
    ...
    if (substring === string.slice(leftToRight, leftToRight + substring.length))
      return true;
  }
  return false;
};
...
const needleInHaystackWithWildcards = (string, substring) => {
  for (let leftToRight = 0; leftToRight + substring.length; index += 1) {
    for (let index = 0; index < substring.length; index += 1) {
      if (
        !(
          string[leftToRight + index] === substring[index] ||
          substring[index] === '_' ||
          '_' === string[leftToRight + index]
        )
      )
        break;
